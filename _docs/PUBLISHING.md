...
現状: **v0.1.0 MVP**。NeoForge 1.21.1 (ルート) / 1.21.4 / 1.21.8 / 1.21.11、Forge 1.21.1 / 1.20.1、Fabric 1.20.1 / 1.21.1 / 1.21.4 / 1.21.8 / 1.21.11 の構成。
...
- [ ] `mod_version` を全サブプロジェクト（ルート含む）の `gradle.properties` で更新（semver）
- [ ] 全サブプロジェクト（ルート含む）で `./gradlew clean build`
- [ ] 全 `build/libs/pingtomapxaeros-x.y.z.jar` のサイズ確認（compileOnly が混入してないこと）
- [ ] 全 jar に `LICENSE_pingtomapxaeros` が同梱されてること (`unzip -l <jar> | grep LICENSE`)
...
- [ ] バージョン番号が各所一致（全 `gradle.properties`・mods.toml/fabric.mod.json・README）
...
| Loaders × MC | NeoForge 1.21.1 (ルート) / 1.21.4 / 1.21.8 / 1.21.11、Forge 1.21.1 / 1.20.1、Fabric 1.20.1 / 1.21.1 / 1.21.4 / 1.21.8 / 1.21.11 |
...
1. 全 `gradle.properties` の `mod_version` を semver で更新
2. `_docs/CHANGELOG.md` 更新
3. 全サブプロジェクト（ルート含む） `./gradlew clean build`
